-- Corrected Users procedure with proper table structure - DS4 Compatible Version
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Parents (DS4 uses parent table instead of contact)
        SELECT 
            LOWER(CONVERT(VARCHAR(32), HASHBYTES('MD5', CONCAT('PAR-', CAST(parentUniqueId AS VARCHAR(50)))), 2)) AS sourcedId,
            'active' AS status,
            lastmodifieddate AS dateLastModified,
            1 AS enabledUser,
            CAST(parentuniqueid AS NVARCHAR(256)) AS username,
            '[{"type":"parentUniqueId","identifier":"' + CAST(parentUniqueId AS NVARCHAR(256)) + '"}]' AS userIds,
            firstname AS givenName,
            lastsurname AS familyName,
            middlename AS middleName,
            CAST(parentuniqueid AS NVARCHAR(256)) AS identifier,
            NULL AS email,
            NULL AS sms,
            NULL AS phone,
            NULL AS agents,
            ...
            NULL AS userProfiles,
            NULL AS agentSourceIds,
            '{"edfi.resource":"parents","edfi.naturalKey.parentUniqueId":"' + CAST(parentUniqueId AS NVARCHAR(256)) + '"}' AS metadata,
            'parent' AS role,
            NULL AS roles
        FROM edfi.parent;
